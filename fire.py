...
MX = 256  # Размер массива пламени
MY = 60
S_FIRE = 4 # Размер ячейки пламени на экране - для масштабирования
...
while running:
    ...
    for x in range(0, MX - 1, 2):
        scr[iout][MY - 1][x] = scr[iout][MY - 2][x] = scr[iout][MY - 1][x + 1] = scr[iout][MY - 2][
            x + 1] = random.randint(0, 1) * 63
    ...
    for y in range(0,
                   MY - 3):  # Текущую плоскость выводим на экран, но не выводим несколько нижних строк по Y, в них случайный "мусор" для генерации пламени
        for x in range(0, MX):
            draw_box(x * S_FIRE, y * S_FIRE, S_FIRE, scr[iout][y][x])
